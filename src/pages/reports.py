...
BG = "#0B0F1A"; CARD_BG = "#111827"; BORDER = "#1F2937"
TEXT = "#E5E7EB"; MUTED = "#9CA3AF"
PRIMARY = "#22C55E"; PRIMARY_H = "#16A34A"
...
class ReportsPage(ctk.CTkFrame):
    def __init__(self, master, switch_page, get_role, export_json_cb, export_pdf_cb):
        ...
        ctk.CTkLabel(head, text="Reports", font=("Segoe UI", 28, "bold"), text_color=TEXT).pack(anchor="w")
        ctk.CTkLabel(head, text="Export machine-readable or presentation-ready files.",
        ...
        card_json = ctk.CTkFrame(row, corner_radius=16, border_width=1, border_color=BORDER, fg_color=CARD_BG)
        card_pdf  = ctk.CTkFrame(row, corner_radius=16, border_width=1, border_color=BORDER, fg_color=CARD_BG)
        card_json.pack(side="left", fill="both", expand=True, padx=(0, 8))
        card_pdf.pack(side="left", fill="both", expand=True, padx=(8, 0))

        # JSON card
        ctk.CTkLabel(card_json, text="Export JSON", font=("Segoe UI", 16, "bold"), text_color=TEXT)\
            .pack(anchor="w", padx=16, pady=(14, 4))
        ctk.CTkLabel(card_json, text="For pipelines and integrations.", text_color=MUTED)\
            .pack(anchor="w", padx=16)
        ctk.CTkButton(card_json, text="Export", fg_color=PRIMARY, hover_color=PRIMARY_H, text_color="#041007",
                      command=self.export_json_cb).pack(anchor="w", padx=16, pady=(10, 14))

        # PDF card
        ctk.CTkLabel(card_pdf, text="Export PDF", font=("Segoe UI", 16, "bold"), text_color=TEXT)\
            .pack(anchor="w", padx=16, pady=(14, 4))
        ctk.CTkLabel(card_pdf, text="Polished, ready to share.", text_color=MUTED)\
            .pack(anchor="w", padx=16)
        ctk.CTkButton(card_pdf, text="Export", command=self.export_pdf_cb)\
            .pack(anchor="w", padx=16, pady=(10, 14))

        # Status + Back
        self.status = ctk.CTkLabel(root, text="", font=("Segoe UI", 11), text_color=TEXT)
        self.status.pack(anchor="w", padx=22, pady=(0, 10))

        ctk.CTkButton(root, text="Back to Dashboard",
