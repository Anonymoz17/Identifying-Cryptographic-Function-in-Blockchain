...
def _start_callback_server(port: int = 8750):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            Handler.result["error"] = q.get("error_description", q.get("error", [None]))[0]
...
def login_with_github(port: int = 8750, timeout_sec: int = 180) -> Tuple[bool, str, Optional[Dict[str, Any]]]:
    ...
    # 2) Begin OAuth
    try:
        resp = _sb.auth.sign_in_with_oauth({"provider": "github", "options": {"redirect_to": redirect_url}})
        auth_url = getattr(resp, "url", None) or (resp.get("url") if isinstance(resp, dict) else None)
        if not auth_url: return False, "No OAuth URL from Supabase", None
