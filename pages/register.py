...
def _make_eye_icon(opened: bool) -> Image.Image:
    ...
    # eye outline (ellipse)
    pad = 3
    d.ellipse([pad, h//2 - 6, w - pad, h//2 + 6], outline=(220, 220, 220, 255), width=2)

    # iris
    d.ellipse([w//2 - 3, h//2 - 3, w//2 + 3, h//2 + 3], fill=(220, 220, 220, 255))
...
class RegisterPage(ctk.CTkFrame):
    def __init__(self, master, switch_page):
        ...
        title = ctk.CTkLabel(card, text="Create your CryptoScope account", font=("Segoe UI", 24, "bold"))
        subtitle = ctk.CTkLabel(card, text="Sign up to get started", font=("Segoe UI", 12))
        ...
        self.create_btn = ctk.CTkButton(
            card, text="Create account", height=40, corner_radius=10, width=ENTRY_W,
            command=self._do_register,
        )
        self.create_btn.grid(row=6, column=0, padx=35, pady=(12, 10), sticky="w")  # same left/right as inputs

        # -------- divider + back link --------
        ctk.CTkFrame(card, height=1, fg_color="#1F2937").grid(row=7, column=0, padx=20, pady=(8, 8), sticky="ew")
        ...
        link_row.grid(row=9, column=0, padx=35, pady=(0, 12), sticky="w")  # align visually with inputs/button
    ...
    def _toggle_password(self):
        ...
        self.eye_lbl.configure(image=self.eye_closed_img_ctk if self._pw_shown else self.eye_open_img_ctk)
    ...
    def _do_register(self):
        ...
        try:
            ok, data = sb_register(email=email, password=password, full_name=full_name, username=username)
        ...
        if ok:
            ...
            self._set_status(data if isinstance(data, str) else "Registration failed.", True)
