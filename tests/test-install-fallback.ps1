...
& "$repo\installation\install-ghidra.ps1" -ArchivePath $zip -Sha256 $sha -AcceptLicense -Version $testVersion -Force -ForceFallback *>&1 | Tee-Object -FilePath $log
...
& "$repo\installation\install-ghidra.ps1" -Uninstall -Version $testVersion *>&1 | Tee-Object -FilePath $log
Get-Content -Path $log -Tail 200 | Write-Host
