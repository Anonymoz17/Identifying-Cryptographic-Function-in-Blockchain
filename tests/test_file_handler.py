...
from file_handler import categorize_file, parse_drop_data, FileHandler

class TestFileHandler(unittest.TestCase):
    def test_categorize_common_extensions(self):
        self.assertEqual(categorize_file('foo.exe', 'application/x-dosexec'), 'binary-pe')
        self.assertEqual(categorize_file('lib.so', 'application/x-executable'), 'binary-elf')
        self.assertEqual(categorize_file('script.py', 'text/x-python'), 'source-python')
        self.assertEqual(categorize_file('archive.zip', 'application/zip'), 'archive-zip')
        self.assertEqual(categorize_file('unknown.bin', 'application/octet-stream'), 'unknown')
    ...
    def test_parse_drop_data_variants(self):
        ...
        self.assertEqual(parse_drop_data('plain.txt other.txt'), ['plain.txt', 'other.txt'])
        # file:// with leading slash should be normalized
        out = parse_drop_data('{file:///C:/path/to/file.txt}')
        self.assertTrue(out[0].endswith('C:/path/to/file.txt') or out[0].endswith('C:\\path\\to\\file.txt'))
